import {
  ...
  ALLOCATIONS_PRESETS_SET,
  SET_ORDERS_COLUMNS,
  GET_ORDER_MODEL,
  LOAD_MORE_ORDERS,
  SHOW_MORE_ALLOCATIONS,
...
export const showMoreAllocations = payload => ({
  type: SHOW_MORE_ALLOCATIONS,
  payload
})
...
const allocationsReducers = {
  [ALLOCATIONS_PRESETS_SET]: (state, presets) => ({
    ...state,
    presets
  }),
  ...
  [SHOW_MORE_ALLOCATIONS]: (state, payload) => {
...
export const allocationsReducer = (state = initialState, action) => {
  let reducer = allocationsReducers[action.type]
