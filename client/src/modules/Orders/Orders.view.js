...
class OrdersList extends Component {
  ...
  componentWillReceiveProps ({ ordersState }) {
    if (
      this.state.preset.columns.length === 0 &&
      ordersState.columnFilter.length > 0
    ) {
      const columns = [...ordersState.columnFilter]
  ...
  componentWillMount () {
    const { presetOn, data, requireUpdate } = this.props.ordersState
    ...
    if (!data || requireUpdate) {
      const { orderBy, filters, type } = this.props.ordersState
  ...
  clearPreset () {
    const { ordersState: { columns } } = this.props
  ...
  applyFilter () {
    const { type } = this.props.ordersState
  ...
  loadMoreOrders () {
    const { ordersLoaded, orderBy, filters, type } = this.props.ordersState
  ...
  contextMenuItems () {
    ...
    // data.id - array, we need to pass it to uri
    return [
      {
        name: 'Go to order details',
        callback: ({ data: { pseudoId } }) => {
          push({
            pathname: `/order-details`,
            search: `id=${encodeURI(pseudoId)}`
          })
        }
      }
    ]
